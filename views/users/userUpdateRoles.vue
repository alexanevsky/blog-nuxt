<template>
  <LayoutContainer>
    <template #breadcrumbs>
      ...
      <LayoutBreadcrumb name="users.titles.updateRoles" :to="{name: 'userUpdateRoles', params: {id: user.alias || user.id}}" :active="true" />
    </template>
    ...
    <LayoutContainerWrapper>
      ...
      <LayoutContainerMain>
        <AppUserEditRoles :user="user" @form-handled="formHandled()" />
      </LayoutContainerMain>
    </LayoutContainerWrapper>
  </LayoutContainer>
</template>
...
export default {
  async asyncData({ params, error, redirect, $repositories, $auth, $t }) {
    ...
    if (user.alias && user.id === parseInt(params.id)) {
      redirect(301, { name: 'userUpdateRoles', params: { id: user.alias } });
      return;
    }
